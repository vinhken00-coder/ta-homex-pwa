...
export default function StaffBottomNav() {
  ...
  const navItems = [
    { href: '/staff/dashboard', icon: 'dashboard', label: 'Dashboard' },
    { href: '/staff/tasks', icon: 'check_circle', label: 'Tasks' },
  ...
  return (
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] z-50 flex justify-around items-center px-4 pb-safe h-20 bg-white/95 backdrop-blur-md rounded-t-2xl shadow-[0_-4px_20px_rgba(0,0,0,0.05)] border-t border-outline-variant/30">
      {navItems.map((item) => {
        const isActive = pathname === item.href || (item.href === '/staff/tasks' && pathname.startsWith('/staff/tasks'));
        return (
          <Link
            key={item.href}
            href={item.href}
